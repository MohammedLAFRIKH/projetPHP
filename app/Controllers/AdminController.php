...
class AdminController {
    ...
    public function __construct(AdminModel $AdminModel, IndexView $IndexView) {
    ...
    public function dashboard() {
        ...
        if (!isset($_SESSION['matricule'])) {
            ...
            header('Location: /apogee_ens/espaceadmin/login');
            exit;
        }
    
        $matricule = $_SESSION['matricule'];
        $model = new AdminModel(); // Adjust this instantiation based on your code
    
        // Check if user data is complete
        $userData = $model->getUserByCNE($matricule);
    
        if ($userData) {
            // User data is complete, show the dashboard
            $this->loginView->showDashboard($isUserConnected=true,$userData,"Plateforme de gestion des étudiants",$userData);
        } 
    }
    ...
    public function showLoginForm() {
        ...
        $this->loginView->renderLoginForm($isUserConnected=true,$error, $success, 'Page de Connexion');
    }
    ...
    public function loginUser() {
        ...
        $user = $this->AdminModel->loginUser($matricule, $password);
        ...
        if ($user) {
            ...
            header('Location: /apogee_ens/espaceadmin/login?error=error');
        }
    }
    ...
    public function forgotten() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $user = $this->AdminModel->checkEmailExists($email);

            if ($user) {
                $resetToken = $this->AdminModel->generatePasswordResetToken($email);

                // Send an email to the user with a link to the resetPassword action and include the $resetToken
                $resetLink = "http://localhost/apogee_ens/espaceadmin/passwords/resetPassword?token=$resetToken"; // Adjust the URL
                ...
                header('Location: /apogee_ens/espaceadmin/passwords/forgotten?success=success');

            } else {
                // The email doesn't exist in the database
                header('Location: /apogee_ens/espaceadmin/passwords/forgotten?error=error');
            }
        } else {
    ...
    public function resetPassword() {
        ...
        $user = $this->AdminModel->getUserByResetToken($token);
    ...
    public function updatePassword() {
        ...
        if ($user) {
            ...
            $this->AdminModel->updatePasswordWithToken($token, $hashedPassword);

            // Clear the reset token after updating the password
            $this->AdminModel->clearResetToken($user['email']);

            // Show a success message or redirect the user to the login page
            header('Location: /apogee_ens/espaceadmin/login?success=success');
        } else {
